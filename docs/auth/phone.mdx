...
To authenticate a user with their phone number, Firebase needs to verify that the authentication requests are coming from your app.
Firebase accomplishes this in either of the following ways.

1. **SafetyNet**: Firebase authentication, using SafetyNet, can verify the device as legitimate with 
   [Android SafetyNet](https://developer.android.com/training/safetynet).
   
   To enable SafetyNet for use with FirebaseAuth
    - In the Google Cloud console, enable the [Android DeviceCheck API](https://console.cloud.google.com/apis/library/androidcheck.googleapis.com) for your project. The default Firebase API Key will be used, and needs to be allowed to access the DeviceCheck API.
    - If you haven't yet specified your app's SHA-256 fingerprint, do so from the [Settings Page](https://console.firebase.google.com/u/0/project/_/settings/general) of the Firebase console. Refer to [Authenticating Your Client](https://developers.google.com/android/guides/client-auth) for details on how to get your app's SHA-256 fingerprint.

2. **reCAPTCHA verification**:if in any circumstances SafetyNet cannot be used,such as user does not have google play services installed on their device, using web platform or testing on emulator, then FirebaseAuth will use reCAPTCHA verification to complete the phone sign in process. Please note that this flow requires that a SHA-1 is associated with your application.

**Note**; The reCAPTCHA flow will only be triggered when SafetyNet is unavailable or your device does not pass suspicion checks. Nonetheless, you should ensure that both scenarios are working correctly.
